...
import { TypedRestBase, TypedRestRoute } from '@typed-rest/core'

export interface TypedRequest<T extends TypedRestRoute> extends express.Request {
  body: T['body']
  params: T['params']
  query: T['query']
}
...
type TypedHandler<T extends TypedRestRoute, Response> = (
  req: TypedRequest<T>,
  res: express.Response
) => Promise<Response>
...
export default function AsyncRouter<APIDef extends TypedRestBase> (
  ...
  > (
    ...
    const route: express.IRouterMatcher<void> = (app as any)[method.toLowerCase()].bind(
      app
    )
